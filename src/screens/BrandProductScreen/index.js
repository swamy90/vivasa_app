...
import { SafeAreaView, View, Text, Image, TouchableOpacity, ScrollView, Dimensions, Modal, FlatList } from 'react-native';
...
const BrandProductScreen = () => {
    ...
    React.useEffect(() => {
        navigation.addListener('focus', () => {
            console.log('focus');
            console.log('BrandProduct', JSON.stringify(params?.params?.item?.id))
            getBrandByProductData();
        });
    }, [false]);

    const getBrandByProductData = async () => {
        const value = await AsyncStorage.getItem('@storage_Key')
        const user = JSON.parse(value);
        console.log('addProductInCart', user?.user?.id);
        setIsLoading(true);
        data.append('brand_id', params?.params?.item?.id) // params?.params?.item?.cat_id)
        data.append('user_id', user?.user?.id) // params?.params?.item?.cat_id)
        const URLs = MY_BASE_URL + "api/brand-product-list";
        ...
        console.log('BrandByProduct', JSON.stringify(submitCustomer));
        if (submitCustomer.status) {
            setIsVisible(submitCustomer.status)
            setBrandProduct(submitCustomer.product);
        } else {
            setIsVisible(submitCustomer.status)
            setCategoryProduct([]);
        }
    ...
    const renderBrandByProduct = (items) => {

        console.log('BrandByProduct', JSON.stringify(items));
        ...
        return (
            <View style={{
                ...
                <TouchableOpacity style={{ padding: 0 }} onPress={() => navigation.navigate('ProductDetailsScreen', items?.item)}>
                    <Image style={{ height: 200, width: '100%', resizeMode: 'cover' }} source={{ uri: MY_BASE_URL + items?.item?.image }} />
                </TouchableOpacity>
        ...
        <SafeAreaView>
            <View style={{ backgroundColor: 'rgb(239,244,250)', height: '100%' }}>
                <View style={{
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <TouchableOpacity onPress={() => sOpenContactsModal()}>
                            <Text style={{ fontSize: 16, fontWeight: '800', color: '#FFFFFF' }}>{params?.params?.item?.brand_name}</Text>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={{ fontSize: 12, fontWeight: '800', color: '#FFFFFF' }}>Other Brand's</Text>
                    ...
                    <View style={{ backgroundColor: '#FFFFFF', borderRadius: 100, padding: 5, marginRight: 5 }}>
                ...
                {isVisible === false ?
                    ...
                    <View style={{ padding: 10, backgroundColor: 'rgb(226,230,229)', flex: 1 }}>
                        <FlatList
                            data={BrandProduct}
                            numColumns={2}
                            keyExtractor={(items) => items.id}
                            renderItem={(items, index) => renderBrandByProduct(items)}
                        />
                    </View>
                }
            </View>
